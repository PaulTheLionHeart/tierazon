...
void FDimension()
    ...
    */
    ...
    switch (filter)
	{
		...
		cRng_x.real(x_rmax - x_rmin);
		cRng_x.imag(0);

		cRng_y.real(y_rmax - y_rmin);
		cRng_y.imag(0);
		...
		switch (nFDOption)
			...
			case 14:
			    rj = fabs(t.real()) * 2000.0 * dFactor;
			    gj = fabs(t.imag()) * 2000.0 * dFactor;
			    bj = (rj + gj) / 2;
			    break;
			...
			case 16:
			    rj = fabs(cFDx.real()) * 1000.0 * dFactor;
			    gj = fabs(cFDy.real()) * 1000.0 * dFactor;
			    bj = sqrt(rj * gj + rj * gj);
			    break;
			...
			case 20:
			    rj = fabs(cFDx.real()) * fabs(cFDy.real()) * 1000 * dFactor;
			    gj = fabs(cFDx.real()) * 1000 * dFactor;
			    bj = fabs(cFDy.real()) * 1000 * dFactor;
			    break;

			default:
			    MessageBox(NULL, "FD Error", "Tierazon", MB_OK | MB_ICONEXCLAMATION);
			    break;
		    }
