...
void Formula_01()
    {
#ifdef REMOVE_TIERAZON_BLOB

while ( n_color++ < maxit && ( mx = z.real() * z.real() ) + ( my = z.imag() * z.imag() ) < bailout )
{
    zz = z.real() * z.imag();
    z.imag(zz + zz + c.imag());
    z.real(mx - my + c.real());
...
void Formula_03()  // Phoenix
    ...
    //c.real(0.563);   // Paul Carlson's point
    //c.real(0.56667); // Ushiki's / Pickover's point
...
void Formula_100()
	...
	{
	z1 = z;
	//z.imag(z.imag() + (sin(z.imag())));
	z.real(z.real() + (sin(z.real())));
	z = z * z + c;
	//temp = z.real();
	//z.imag(z.real());
	zd = z - z1;
