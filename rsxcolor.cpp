...
void Generalized_Coloring_Method3()
    {
    //temp = fabs(sin(1 + z.real() * z.real() * 100))*
      //     fabs(sin(1 + z.imag() * z.imag() * 100))*
	//      100*dFactor_2;
...
void Generalized_Coloring_Method20()
    ...
    c.real((fabs(c.real() / x_size)));
    c.imag((fabs(c.imag() / y_size)));
    //	z = c;
    ...
    rj += ((sqrt(c.real() * c.real() + c.imag() * c.imag())) * dFactor_2);

    gj += ((sqrt(xsav_ * xsav_ + ysav_ * ysav_)) * dFactor);
    gj += ((sqrt(c.real() * c.real() + c.imag() * c.imag())) * dFactor_2);

    bj += ((sqrt(xsav_ * xsav_ + ysav_ * ysav_)) * dFactor);
    bj += ((sqrt(c.real() * c.real() + c.imag() * c.imag())) * dFactor_2);
...
void Generalized_Coloring_Method22()
	...
	fabs(c.imag() / y_size)))) * dFactor_2;

    gj += (((fabs(c.real() / x_size) +
	fabs(c.imag() / y_size)))) * dFactor_2;

    bj += (((fabs(c.real() / x_size) +
	fabs(c.imag() / y_size)))) * dFactor_2;
...
void Generalized_Coloring_Method25()
    ...
    rj += 2000 * dFactor_2 * fabs(ysav * c.imag());
    gj += 2000 * dFactor_2 * fabs(xsav * c.real());
